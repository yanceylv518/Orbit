...
本节记录在当前引擎实现（`src/ddg/engine.py`）与前面章节（§8、§10、§11、§12）比对后发现的逻辑缺口，以及需要补齐的设计规则。这些是策略层面必须先在设计上定义清楚、再落地代码的问题，优先级从高到低排列。
...
1. 明确风控结果到动作的映射：`ONLY_REDUCE` 时禁止所有 ADD 类动作；`PAUSE_SYMBOL` 时整个 symbol 当 tick 跳过所有事件。
...
需要补的设计规则：在 §12.5 增加 symbol 级硬止损：当单币种回撤触及上限时，**平掉亏损腿（de-hedge）而不是仅暂停**，并写入风控事件。

### 21.5 趋势确认应引入斜率/时间维度（§8.3 补充）

问题：当前趋势确认只看「相对固定 base 的价位位移」，无法区分慢速阴跌 4% 和数分钟暴跌 4%。叠加 21.1 的陈旧 base，系统会长期把普通漂移误判为趋势。
...
2. 当 `restore_loss_side_only_to_base = true` 且亏损腿已恢复到 base 时，`add_notional` 会 ≤ 0，导致**整次利润搬运（含减盈利腿）被整体跳过**，即持续趋势中止盈会停止。需明确这是「刻意让利润奔跑」的设计，还是需要拆分为「即使不加仓也允许单独减盈利腿止盈」。
...
每次利润搬运 = 2 笔 taker + 2 笔滑点；base 小、阈值低时高频小额搬运容易被费用吃光。建议在 §11.4 / §8.1 的 `min_net_profit_usdt` 约束中，要求净利润不仅覆盖**本次**成本，还要覆盖**下一次反向平仓的预估成本**。
...
§13、§15 使用 `config.yaml`（YAML）描述配置，但实际实现采用 JSON：`config.sample.json`、`config.local.json`。后续按 JSON 对接，§15 的 YAML 示例仅作字段结构参考。

### 22.2 模块结构：扁平化实现

§13 描述了 `strategy/ execution/ exchange/ accounting/ risk/ storage/ reporting/ admin/` 多层目录，V1 实际采用扁平结构 `src/ddg/`（`engine.py`、`planning.py`、`storage.py`、`server.py`、`app_state.py`、`binance.py`、`reporting.py`、`auth.py`、`config.py`）。多层结构为后续演进方向，当前不作为验收依据。
...
Binance API Key/Secret 的本地加密使用 Windows DPAPI（`src/ddg/binance.py`），**仅在 Windows 可用**。当前开发/部署环境为 Linux，保存加密 Secret 会抛 `CredentialError`，对应单测在非 Windows 上被跳过。Linux 环境需另做跨平台凭证后端，或改用环境变量引用（`env:` 前缀）模式。

### 22.5 运维脚本平台限制

`scripts/` 下仅有 `.cmd` / `.ps1`（Windows），README 启动命令为 PowerShell + `C:\Users\...` 路径；当前 Linux 环境无对应 bash 脚本，且未安装 `node`（进度文档中的 `node --check web/app.js` 验证在 Linux 上无法复现）。需要补一套 Linux/bash 启动与校验说明。
