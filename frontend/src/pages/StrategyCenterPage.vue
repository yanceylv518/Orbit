<template>
  <section class="page active strategy-detail-page">
    ...
    <article v-if="!isTrend" class="panel strategy-section">
      ...
      <ol class="strategy-five-answers round-summary"><li><span>1</span><div><h4>可用市场</h4><p>通过数据与交易性准入</p></div><b>{{ latestRound.market_count || 0 }}</b></li><li><span>2</span><div><h4>初步命中</h4><p>当轮规则识别的全部信号</p></div><b>{{ latestRound.detected_count || 0 }}</b></li><li><span>3</span><div><h4>新增入账</h4><p>去重后写入模拟账</p></div><b>{{ latestRound.new_signal_count || 0 }}</b></li></ol>
    </article>
