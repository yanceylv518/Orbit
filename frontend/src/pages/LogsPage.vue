    ...
    <article class="panel">
      ...
      <div class="event-log">
        <div v-for="event in events.slice(0, 24)" :key="event.id" class="event-item">
          <strong :title="`系统原始值：${event.event_type}`">{{ eventLabel(event.event_type) }} · {{ event.symbol }}</strong>
            ...
            <table>
              ...
              <tbody>
                <tr v-for="trade in event.trades || []" :key="trade.id">
                  <td :title="`系统原始值：${trade.action}`">{{ enumLabel(trade.action) }}</td>
                  <td :title="`系统原始值：${trade.position_side}`">{{ enumLabel(trade.position_side) }}</td>
...
import { enumLabel, eventLabel, stateLabel } from "../domain/labels.js";
