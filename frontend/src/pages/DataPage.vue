        ...
        <article class="panel data-history-card">
          ...
          <div v-else class="data-history-list">
            <article v-for="run in historyRuns" :key="run.id" class="data-history-row">
              ...
              <div class="data-history-result"><span class="history-status" :class="run.status">{{ statusText(run) }}</span><small v-if="run.status === 'failed'">{{ failureText(run) }}</small><small v-else>{{ run.status === 'cancelled' ? '已完成内容保留，可继续更新' : run.message || '数据已校验并可使用' }}</small></div>
            </article>
          </div>
        </article>
    </section>
...
const category = ref("全部分类"); const scope = ref("全部范围"); const selectedCoin = ref(null); const sortKey = ref("change_24h_pct"); const sortDirection = ref("desc");
...
const cutoff = computed(() => summary.value?.dataset_cutoff_ms ? new Intl.DateTimeFormat("zh-CN", { timeZone: "UTC", year: "numeric", month: "numeric", day: "numeric" }).format(new Date(summary.value.dataset_cutoff_ms)) : "尚未生成");
...
function statusText(run) { if (run.status === "succeeded" && run.update_result === "NO_CHANGES") return "检查完成"; return { succeeded: "更新成功", failed: "更新失败", cancelled: "已停止" }[run.status] || "已结束"; }
function updateAmount(run) { if (run.update_result === "NO_CHANGES") return "没有新增数据"; if (run.added_partitions) return `新增 ${Number(run.added_partitions).toLocaleString("zh-CN")} 个日分区`; if (run.contract_count || run.partition_count) return `覆盖 ${Number(run.contract_count || 0).toLocaleString("zh-CN")} 个合约 · ${Number(run.partition_count || 0).toLocaleString("zh-CN")} 个数据分区`; if (run.completed_items) return `已处理 ${Number(run.completed_items).toLocaleString("zh-CN")} 个数据文件${run.completed_bytes ? ` · ${formatBytes(run.completed_bytes)}` : ""}`; return run.status === "failed" ? "未完成" : "没有新增数据"; }
