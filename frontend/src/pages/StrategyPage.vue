...
const familyEntries=computed(()=>Object.entries(store.signalDesk?.recent_samples_by_family||{}));const count=p=>signals.value.filter(r=>p.test(String(r.family_id||r.family||r.type||""))).length;const rollingCount=p=>Object.entries(store.signalDesk?.rolling_30d_by_family||{}).reduce((sum,[family,value])=>sum+(p.test(family)?Number(value):0),0);const dailyAverage=id=>Number(store.signalDesk?.rolling_30d_daily_average_by_family?.[id]||0);const sample=p=>signals.value.find(r=>p.test(String(r.family_id||r.family||r.type||"")))||familyEntries.value.find(([family])=>p.test(family))?.[1]||null;const prices=s=>[...(s?.chart_before||[]),...(s?.chart_after||[])].map(r=>Number(r.close)).filter(Number.isFinite);const familyControl=id=>store.signalDesk?.operations?.family_controls?.[id]||{enabled:true};
const cards=computed(()=>{const trendStarted=store.state?.trend_forward?.status&&store.state.trend_forward.status!=="NOT_STARTED";const make=(slug,familyId,name,summary,pattern,empty)=>{const s=sample(pattern);const control=familyControl(familyId);const enabled=serviceEnabled.value&&control.enabled;const reason=!control.enabled?(control.disabled_reason||"该类提醒已停用"):serviceEnabled.value?"扫描服务正常运行":store.signalDesk?.health?.status==="NOT_DEPLOYED"?"扫描服务尚未部署":"提醒服务已关闭";return{slug,name,kind:"推给你决定",summary,signal:s,points:prices(s),emptyText:empty,status:enabled?"提醒已启用":"提醒未启用",color:enabled?"green":"orange",reason,metrics:[{label:"今日推送",value:`${count(pattern)} 个`},{label:"近 30 天",value:`${rollingCount(pattern)} 个`},{label:"实际日均",value:`${dailyAverage(familyId).toFixed(2)} 个`}]}};return[{slug:"trend",name:"趋势策略",kind:"自动执行",summary:"12 个主流币，谁在涨就跟谁，每周调一次仓。",points:equityPoints.value,emptyText:"等待净值记录",status:trendStarted?"运行中":"尚未启动",color:trendStarted?"green":"orange",reason:trendStarted?"按每周目标自动调仓":"尚未开始模拟计分",metrics:[{label:"当前持仓",value:`${holdingCount.value} / 12`},{label:"近 30 天",value:"观测不足"}]},make("breakout","BREAKOUT_MOMENTUM","突破信号","价格冲破近期高点、成交明显放大时提醒你。",/break|突破/i,"尚无突破样本"),make("oversold","OVERSOLD_REBOUND","高位回调","上涨途中一次急跌后出现企稳迹象时提醒你。",/oversold|超跌/i,"尚无回调样本"),make("strength","SUSTAINED_STRENGTH","持续强势","价格连续走强、成交持续活跃时提醒你。",/strong|强势/i,"尚无强势样本")]});
