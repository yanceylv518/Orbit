...
**另行登记**：`8d5cf2c`~`842fe89`（实盘启用向导、杠杆校验、V3 逐仓 3x）为实盘资金相关改动,尚未经 Claude 独立验收,建议单独安排一轮验收（重点:V3 清单倍数与回读校验、向导双重确认的摩擦等价性、`fapi/v3` 接口迁移正确性）。
...
### 任务 DATA-1：含退市合约的全市场历史数据集 + 时点币池（**优先级：暂缓**，用户 2026-07-31 决定先聚焦部署与实盘；研究线重启时再执行,规格保持有效）
