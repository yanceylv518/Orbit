...
**三项修正**：① **DATA-1 复活为硬前置**——本文 §2.2 明确禁止「依赖仍存活币种回推历史币池」,这在字面上就需要含退市合约的全量历史（DATA-1,当前状态暂缓）;阶段 2/3 的历史币池重建与 replay 没有它无法诚实进行,且分钟级研究还需将 DATA-1 扩展到 1m 粒度（数据量升至百 GB 级,需单独评估存储）。② **历史点差/深度数据源可行性验证**——币池政策与成交性模拟依赖历史 bid/ask 与深度,公开归档仅部分提供（bookTicker/bookDepth 覆盖范围与起始时间需实测）;此项不落实,§9 的「使用当时 bid/ask」是无源之水,须在冻结 UniversePolicy 前完成数据源核查并写入证据。③ **排期不得挤占现有主线**——TB4 paper 前向与 LIVE-SMALL 的部署运行仍是第一优先级;本架构各阶段执行前须逐阶段经用户显式排期确认,不默认连续推进。
...
- **目标**：在平台投入前回答——**突破/动量族**与**超跌反弹族**（5m/15m 信号周期）扣除保守成本后是否存在正期望迹象。这是筛查不是准入:PASS 只解锁「值得为它建平台并进入组合级回测研究」,不授权任何交易。
- **方法（先预注册后跑数,四护栏全套）**：
  ...
  5. 统计门（C8 风格,跑数前冻结）：每族每定义最少事件数、成本后均值为正、Wilson 95% 下界 > 0、固定种子 bootstrap 确认;聚合过门且不得仅靠单一流动性层/单一年份撑起。
...
**扩展（2026-08-10）**：在原规格基础上,K 线粒度从仅 4h 扩为 **4h + 15m + 5m**（R-0 信号周期所需;1m 暂不拉,待 R-0 结论后评估）。存储量修正估算:5m≈4h 的 48 倍、15m≈16 倍,合计预计 **20–40 GB**,普通开发机可承载;下载工具须支持按粒度分批与断点续传。其余规格（合约全集枚举、退市元数据、`universe_at` 无未来泄漏、质量报告、manifest 指纹、LUNA 存在性测试）不变,仍然有效。
