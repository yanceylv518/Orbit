...
**UI-P1 完成记录（2026-07-14）**：研究平台前端已接入 UI-P0，只读展示数据目录、候选墓地、冻结定义、固定 bar、锁箱溯源和结构化结果证据；桌面/移动端及筛选、候选切换均完成真实浏览器验收。下一项为 UI-P2 job runner 与四条研究护栏的写流程实现。
...
- `npm run check` 通过。
- `npm run build` 通过。
- Python 单元测试及 API 契约测试：`265 tests OK`。
- `git diff --check` 通过。
