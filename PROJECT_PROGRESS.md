...
- **改动**：
  1. 新增**只读**执行清单投影 `execution_checklist`：输入实盘资金规模（配置项或查询参数，如 `live_capital_usdt`），对每币输出：方向（LONG/SHORT/FLAT）、目标名义 USDT（`weight × capital`）、按最新收盘价折算的目标数量、与上一次清单的名义变化额。资金规模仅用于换算展示，**不进入 runner 状态、不影响账本与对齐性质**。
...
- **验收**：① 清单为纯投影，重复计算幂等、不改 runner/账本状态（保持 TB4-A/TB4-B 对齐与哈希链性质）；② 权重×资金×价格换算有单测（含 FLAT/反向翻转/资金规模变化场景）；③ 全库 grep 无新增下单调用；④ 前端只读展示；⑤ 测试全绿。
