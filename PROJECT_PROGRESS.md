...
**说明（诚实，务必记住）**：① 前向测试**自动修掉了回测的两个偏差**——真实成交价/滑点（执行真实性）和「只交易活着的币」（幸存者偏差，前向是真前瞻），**只剩「行情 regime」这一个偏差只能靠时间解决**；② TB4 交付的是「跑起来的测试」，**真结论要等几个月**，这期间最重要的动作是**什么都别动**；③ 这也终于回答用户那个问题——「回测表现是否真实准确」，**paper 前向就是那把尺子**。
...
| 2 | **UI-P0 只读研究 API**（数据目录 + 候选注册表[只追加/冻结不可改] + 结果读模型） | 本文「研究平台（方向1）前端化计划」 | 高 |
| 3 | **UI-P1 研究前端（只读）**（数据目录 / 候选墓地 / 候选明细对照固定 bar） | 同上 | 中 |
| 4 | **UI-P2 job runner + 触发**（网页填预注册→冻结→点一下跑缓存数据→看进度→出 verdict；四护栏 UI 化） | 同上 | 中（依赖 P0） |
...
- `npm run check` 通过。
- `npm run build` 通过。
- Python 单元测试及 API 契约测试：`251 tests OK`。
- `git diff --check` 通过。
