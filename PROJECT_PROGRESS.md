...
### SIG-2 阶段实现记录：信号台基础闭环（2026-08-14，Codex）

**本阶段目标**：在不改变 SIG-1 信号定义和模拟账的前提下，先落地可见、可点、可追溯的信号台基础闭环。本节是实现记录，以上方最新 SIG-2 正式任务为唯一验收契约。

**冻结范围**：

- 新增“信号”一级页面，包含信号台和信号复盘；信号卡以紧凑 K 线为主体，在同图展示成交量、进场、止损和小型方向箭头，逐根可查看涨跌幅与振幅。
- 人工配对只允许对正式候选登记“我做了 / 我跳过”；已做信号可补录完整人工退出，但不自动连接 Binance 成交、不自动下单。
- 强制四条约束：选择时必须带有效止损；一个信号只允许一笔人工成交；同交易对同方向止损后冷静 4 小时；候选范围外的交易不得登记。
- 固定三本互不覆盖的账：全部信号模拟、我选择的信号模拟、我的实际成交。
- SIG-1 与 SIG-2 分账存储并各自保持只追加哈希链；源账缺失或校验失败时诚实降级，不制造示例信号。

**实现记录（Codex，2026-08-14）：第一阶段已完成，SIG-2 整体验收未完成。** 后端新增管理员信号台读取、人工决定和人工成交接口，以及独立 `ORBIT_SIG2_INTERACTION_LEDGER_V1`；运行时信号开始保存最近 48 根已收盘 K 线快照，模拟退出保存入场至退出的后续路径。前端完成信号台、决定/退出登记、三账试算与桌面/390px 窄屏布局；被截断候选继续展示其模拟记录，但交互入口被锁定。旧 SIG-1 记录没有图形快照时只显示明确空态。阶段设计见 `docs/design/SIG2_SIGNAL_DESK.md`。验收：后端全量 `505 passed`（仅 2 条既有 Pydantic 弃用警告），前端 check/build 通过，`MODULAR_BASELINE_PASS`，TB4 受保护文件零改动；桌面与 390×844 窄屏已完成浏览器走查。**按最新正式契约仍待补齐**：Pushover 配置与测试通知、服务启停和完整健康观测、真实成交自动配对与计划外交易识别、专用子账户防误绑、连续亏损熔断与 24 小时放宽延迟、推送深链/故障告警/备份清单；三账正式复盘归 SIG-3，本阶段页面仅作试算，不作为 SIG-2 完成依据。
