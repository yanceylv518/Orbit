...
**扩展（2026-08-10;粒度最终定稿:用户拍板 15m 以下不用）**：在原规格基础上,K 线粒度定为 **4h + 15m 两档**,不下 5m/1m。存储量估算:15m≈4h 的 16 倍,合计预计 **8–15 GB**。下载工具须支持按粒度分批与断点续传。其余规格（合约全集枚举、退市元数据、`universe_at` 无未来泄漏、质量报告、manifest 指纹、LUNA 存在性测试）不变,仍然有效。**连带约束**：R-0 及后续短线研究的信号周期下限为 15m;架构文档 §2.1「底层保存 1m」的目标相应收窄——若 R-0 通过后确需更细粒度,须另行评估立项,不默认下载。
