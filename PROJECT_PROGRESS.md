...
## UI 总重构（2026-08-15 用户与 Claude 完整讨论后定稿；本节取代此前全部导航与页面结构决定）

**背景**：连续多轮生产反馈（垃圾信号、黑盒规则、参数由 Claude 定、页面无图、内部代号无意义、研究与策略混为一页）指向同一根因——**系统按「研究工程的展示界面」建造,而它应是「用户每天使用的交易工具」**。本节把讨论结论固化为唯一的结构真源;与本节冲突的旧决定（UI-R1/UI-R2/PAGE-1/PAGE-2/PAGE-3 中的导航与页面归属部分）**一律以本节为准**。

### 一、导航（七项，全部两字，按使用频率排序）

```
首页  实盘  信号  策略  研究  数据  账户
```
- **两字为硬约束**：侧栏竖排需左对齐可扫;将来新增菜单同样须能压成两字——若某项非两字说不清,通常说明该分类本身没切干净。
- **复盘不设独立入口**：量化复盘并入「实盘」Tab、信号复盘并入「信号」Tab（复盘天然跟着业务线走,不存在通用复盘）。
- **数据保留独立入口**（用户 2026-08-15 定）：数据不只服务研究——币种列表、单币查询、是否在扫描范围内属日常运营判断,与研究无关。
- **研究与策略必须分开**（用户 2026-08-15 定）：研究＝**尚未成型**的探索方向;策略＝**已成型可运行**的规则。二者状态与用法不同,不得合并为一页 Tab。

### 二、全局骨架（七页统一，学一次通用全站）

**顶栏常驻**：品牌 ｜ **全局状态灯**（实盘/行情是否正常,任何页面都能看到）｜ **消息铃 + 未读数** ｜ 当前用户。

**页内统一结构**：
```
问题式标题 + 一句话答案
四格状态条
主内容区（主列 2/3 · 侧列 1/3；图在前、数字在后）
次要内容进 Tab
技术详情最底部，默认收起
```
**贯穿原则**：① **图在前、数字在后、参数最后**;② 凡用户不能动手的内容不占首屏;③ **正常态极简、异常态细节自动浮出**;④ **界面零内部代号**（M0/F1/G*/V*/TB*/R-0/RB-*/RC-*/SIG-*/MOD-*/PAGE-*/DATA-* 等一律禁用,折叠隐藏亦不允许）;⑤ 研究流程术语（预注册/锁箱/冻结/verdict 等）不得出现在用户界面。

### 三、首页（新增，此前被砍，现因多条业务线并行而必需）

只回答每天睁眼时的三个问题,**正常的日子应几乎是空的**：
1. **系统正常吗**——四格：自动实盘、信号扫描、行情数据、消息（未读错误数）;全绿则一句「一切正常」。
2. **有没有事等我处理**——**仅在有事时出现整块**：未读错误、持仓核对不一致、今日待决策信号数、数据超 48 小时未更新。
3. **钱怎么样了**——自动策略累计盈亏、手动交易累计盈亏两个数字,点击进各自复盘。
   另加**今日信号摘要**（几个、最强的是什么,点击进信号台）。
   **首页纪律**：只放「需要行动」与「一眼确认正常」的内容;任何需要琢磨才能理解的东西一律不放首页。

### 四、消息模块（新增，用户 2026-08-15 提出）

**双通道分工**：**Pushover ＝ 打断你**;**站内消息 ＝ 攒着等你看**;二者同一消息源。
- **入口**：顶栏铃铛 + 未读数 → 点开预览列表（时间/级别/一句话）→ 点条目进详情（完整信息 + 相关跳转,如「去看那笔核对差异」）;支持已读/未读与全部已读。
- **分级与推送**：**错误级**（系统故障或资金风险）→ 站内 + **自动推 Pushover**;**重要级** → 站内,是否推送由用户在设置中选;**提示级** → 仅站内。
- **消息源（首版）**：数据更新失败/连续失败、行情中断、扫描停止、实盘执行异常、订单失败、持仓核对不一致、回撤接近或触及停机线、急停、协议停机、推送失败、配置变更留痕。
- **信号推送单列一类**：每条 Pushover 信号推送在站内留记录,便于回查「昨天到底推了什么」;与故障消息分列,可按类型筛选。
- **消息只追加保留、可翻查**（供事后复盘「那天发生了什么」）。**故障告警不占用每日 3 条信号推送额度。**

### 五、数据页（本轮完整定稿）

**核心问题**：数据够新、够全吗？

**四格状态**：覆盖币种数 ｜ 更新到何时 ｜ 数据缺失情况 ｜ 时间尺度（15 分/1 时/4 时）。

**五个模块**：
1. **历史数据仓**——覆盖范围、更新时点、完整性、缺口清单、「检查并更新」按钮。
2. **币种列表**（主列）——**默认按 24 小时成交额降序**;字段：24 小时成交额、上市时间/天数、**分类**、**是否在扫描范围内（不在时直接显示原因,如「成交额不足」）**、最近 7 天涨跌、当前长周期状态、数据完整性;支持按分类与是否在范围内筛选。
   - **分类规则**：主流币＝**系统给默认名单、用户可编辑**（初始填 BTC/ETH/BNB/SOL/XRP/DOGE/ADA/AVAX/LINK/DOT/LTC/BCH 等）;股票类与大宗商品按合约命名规则自动识别;其余归山寨币。**不做更细分类。**
3. **单币查询**（侧列固定）——输入币种 → 走势图 + 基本信息 + 数据完整性;可查列表外的币。
4. **后台任务区**——**无任务时整块隐藏**;有任务时显示阶段、进度、当前对象、错误数。
5. **最近历史任务**——只显示最近几条,详情进 Tab。

**每日定时更新**：
- 时间 **UTC 01:00**（币安日归档需 UTC 整日结束后方稳定发布,过早会扑空）;
- 内容：增量下载昨日分区 → 校验 → 聚合 1 时/4 时 → 更新数据集状态与质量报告;
- **失败自动重试**（间隔 1 小时,最多 3 次）;**连续两天失败 → 推 Pushover 告警**（走故障通道,不占信号额度）;页面显示「上次成功更新时间」,超 48 小时变红;
- **成功不推送**（避免噪音麻痹）;
- 新数据与既有数据冲突（如交易所修正历史）时**停下告警,不得静默覆盖**;
- **与实盘完全隔离**：本任务任何失败均不得影响实时行情与实盘执行（须有测试守护）。

**数据存放（用户 2026-08-15 定）**：
- **服务器保留全量历史数据**——网页上的回放、图表、单币查询、研究试条件都依赖它,不放服务器等于研究功能全是空的;
- **本地同时保留一份**——跑大批量计算时用,亦为天然备份;
- **服务器直接下载**（该机可达币安归档）,不从本地上传;首次全量数小时,之后每日增量数十 MB;
- **两边以数据集校验码比对一致性**,确保网页回放结果与本地计算可对齐;
- **K 线不入数据库**：K 线为不可变、只整段读取、丢失可重下的档案,放文件;数据库只存数据集状态（版本、覆盖、更新时间、质量报告、缺口）——数十条记录而非上亿行。

### 六之一、策略页（2026-08-15 讨论定稿，本批之后的下一批交付）

**核心问题**：我在用哪些规则？**设计目标**：列表本身就要让用户看懂每个策略在干什么,而不是「名字 + 状态」式的索引。

**列表按性质分两组**（这个区别比任何参数都重要——哪些在替你花钱、哪些只是递情报）：
```
自动执行（机器决策，你只需看报告）
  └ 趋势策略
推给你决定（系统筛选，你来判断）
...
**每行四个信息，缺一不可**：
1. **一句话说清它在找什么**——纯人话零术语。例：趋势策略「12 个主流币,谁在涨就跟谁,每周调一次仓」;突破信号「价格冲破近期高点、成交量明显放大时提醒你」;超跌反弹「短时间暴跌后出现企稳迹象时提醒你」。**禁止出现指标名与参数**。
2. **一张缩略图（关键）**——自动策略放**净值曲线**;信号策略放**形态示意图**（取自真实历史样本,展示「这类信号长什么样」）。**图是让用户认得出策略的主要手段**,纯文字记不住四个策略的区别。
3. **它现在在干嘛**——自动策略:持仓数/总数、近 30 天表现;信号策略:今日推送条数、近 30 天累计条数。
4. **状态与原因**——停用的必须写明停用原因（如「下跌趋势中亏损多」),否则数月后无从回忆当初为何关闭。

**四项待定事项的处置（Claude 建议，实现时按此，用户可随时改）**：
- **累计盈亏不进列表行**——属复盘内容,列表只放「近 30 天表现」这类概览;
- **启用/停用开关在详情页**,不在列表行（防误触）;**自动策略的停用须二次确认**（等同停止真钱交易）;
- **策略页不设「新建」入口**——策略只能从研究页「保存为策略」毕业而来,以保证「先看回放再决定」这一步不被跳过;
- **详情页六节骨架统一**（在找什么/用哪些指标/参数取值/前置过滤/后置过滤/进出规则）,**自动策略额外增加两块:当前持仓图 + 权益曲线**。

**验收**：① 列表分两组且组名为人话;② 每行含一句话描述、缩略图、当前动态、状态与停用原因;③ 描述与列表行**零指标名零参数零内部代号**;④ 详情六节齐全,自动策略含持仓图与权益曲线;⑤ 无新建入口;⑥ 停用开关在详情页且自动策略二次确认;⑦ 全量测试绿 + 基线 PASS + TB4 零触碰;⑧ 桌面与窄屏实测。
- **验收结论（Claude，2026-08-15）：部分通过,四项打回（`2c8e4e9`）。**
  - **合格部分**：一句话描述写得到位且是人话（如「让多个时间尺度共同判断涨跌,并让波动较小的市场承担更多仓位」「寻找成交突然活跃、价格冲出近期区间的机会」）;详情页五节标题人话化（「这个策略在找什么」「找信号之前排除什么」「找到信号之后筛掉什么」「怎样进场与出场」）;自动策略的权益曲线与持仓图组件已实现;无新建入口;`517 passed`、`MODULAR_BASELINE_PASS`、TB4 零触碰。
  - **打回一 · 列表未分两组**：现为平铺列表,仅靠卡上 `自动策略/信号策略` 标签区分。规格要求按「自动执行（机器决策,你只需看报告）」与「推给你决定（系统筛选,你来判断）」**分组呈现**——这是全页最重要的区别（哪些在替你花钱、哪些只是递情报）,标签不能替代分组。
  - **打回二 · 列表卡无缩略图（最重要的缺失）**：规格明写「**图是让用户认得出策略的主要手段**」——自动策略须放净值曲线缩略图,信号策略须放形态示意图（取自真实历史样本）。当前列表页**一张图都没有**,与用户「页面没有任何图形」的核心批评直接冲突。
  - **打回三 · 列表行出现指标与参数,违反明确禁令**：卡片含「核心指标：趋势投票 · 近期波动 · 组合风险」「当前参数：观察 14/28/56/84/168 天 · 每 7 天调仓 · 目标波动 10%」两行。规格：**列表行禁止出现指标名与参数**,二者属详情页第 2、3 节。
  - **打回四 · 详情页缺「参数取什么值」一节**：六节实现了五节,参数节缺失（可调项须链到配置面板,不可调项须注明原因）。
  - **另**：停用原因展示未见（当前无停用策略,待有停用状态时须显示）;本批无新增测试,策略卡数据组装建议补最小单测。
  - **补交验收（Claude，2026-08-15，`8e38265` + `883aa58`）：四项打回中三项已补齐,一项仍缺。** ✅ 分组已实现（「自动执行」/「推给你决定」两组）;✅ 缩略图已实现（新增 `StrategyThumbnail.vue`,画真实走势并标注信号位置,列表两处使用）;✅ 列表行的「核心指标/当前参数」已移除;`518 passed`。❌ **详情页仍缺「参数取什么值」一节**——现为五节（在找什么/用哪些指标/找信号之前排除什么/找到信号之后筛掉什么/怎样进场与出场）,参数节未出现。该节须列出逐项当前值,可调项链到配置面板、不可调项注明原因。
  - **Claude 自行修复（2026-08-15，应用户要求）**：参数数据本已存在,但被 Codex 放进底部「技术详情」折叠区,而非正文第 3 节。已**将参数提升为正文 03 节「参数取什么值」**（信号策略附「去配置面板调整」按钮,自动策略注明「与正在运行的自动交易绑定,页面只读,调整需重新授权」）;技术详情区改为放真正的排障信息（策略编号、运行方式、数据来源）。现六节编号 01–06 连续无跳号,`518 passed`、`MODULAR_BASELINE_PASS`、结构标签配平。**策略页四项打回至此全部闭合。**

### 工作方式修正（2026-08-15，用户质疑「说得很好做得很差」后 Claude 复盘）

**用户原话**：「我说了这么多,你和 Codex 都不能达到我的要求,你俩说的都很好,但是做的很差,是不是有什么原因。」**问题成立,根因在 Claude 的工作方法,记录并修正:**
1. **用文字描述视觉需求,必然失真**。全项目交付质量最高的一次是提供了可运行样稿 `PAGE2_MOCKUP.html` 的那批;此后各批仅有文字规格,反复出现「结构对但没有图/细节走样」。**修正:凡涉及页面与图形的任务,先产出可运行 HTML 样稿作为验收基准,再交 Codex 实现,不再以文字为准。**
2. **规格写成了讨论记录**。本文件已达 1900+ 行,层层追加、互相引用、含历史修订痕迹;Codex 需先从中考古出「本次到底做什么」。**修正:每个任务单独出一页自足的任务卡（当前有效内容、无历史包袱）,`PROJECT_PROGRESS.md` 只留结论与验收记录。**
3. **原则宣言不是可交付物**。如「图是让用户认得出策略的主要手段」是判断,不是规格;Codex 无从得知画什么图、数据取自哪里、多大尺寸。**修正:每个视觉元素给出数据来源、尺寸、状态（空/正常/异常）三要素。**
4. **需求在快速演进,Codex 可能在按中间版本施工**。本轮导航改三次、策略页结构改两次。**修正:讨论期间不发指令,讨论收敛后一次性发布带版本号的任务卡。**
5. **验收发生在事后**。**修正:样稿即验收标准,开工前用户先看样稿确认,减少事后返工。**

### 六、本批实现范围（交付 Codex）

**只实现「骨架 + 数据页 + 消息模块」**。**其余菜单一律保持现状不动**（用户 2026-08-15 明确：其他菜单都先按以前的）——**不出空页占位、不改内容、不删旧页**。
- **迁移方式：新增在前、删除在后（用户 2026-08-15 定）**。本轮及后续每个菜单的重做,都采取「**先加新菜单/新页面并跑通,最后统一删除旧的**」的顺序——不得在新页尚未可用时先拆旧页,避免出现任何功能真空期。旧页面在其对应新页面验收通过前**保持可访问**;全部菜单完成后再统一清理一次。
- **本轮导航的实际形态**：新增 `首页`、`数据`（新版）两项并入七项骨架;`实盘/信号/策略/研究/账户` 暂时仍指向**现有页面**,外观按新骨架统一,内容不动。旧的多余入口（工作台、执行计划、币种视图、报表等）**本轮不删**,待全部新页就位后统一退役。
- **验收**：① 导航七项两字、顺序正确;② 顶栏状态灯与消息铃在所有页面常驻;③ 数据页五模块与四格状态齐全、币种列表字段与排序符合上文、无任务时任务区隐藏;④ 定时更新任务、失败重试与连续失败告警有测试;⑤ 消息模块分级、推送规则、只追加与未读计数有测试;⑥ **新增页面零内部代号**（grep 断言;既有页面本轮不强制清理,留待各自重做时处理）;⑦ **既有菜单功能零回归**（现有页面照常可用,有测试守护）;⑧ 全量测试绿 + 基线 PASS + TB4 零触碰;⑨ 桌面与 390×844 实测。
- **验收结论（Claude，2026-08-15）：通过（`023842a` + `4aab67d`）。** 逐条核实:① 导航恰为七项两字、顺序为 首页/实盘/信号/策略/研究/数据/账户;② 顶栏常驻**全局状态灯**（点击直达实盘）与**消息铃含未读数**,在所有页面可见;③ 数据页含币种列表（**按 24 小时成交额降序**、分类与扫描范围双筛选、**扫描状态列附 `scan_reason` 说明不在范围的原因**）、单币详情（含上市时间、分类、扫描状态）、实时行情流状态;④ **定时更新有专测**——`test_daily_update_retries_hourly_and_alerts_after_two_failed_days`（每小时重试、连续两日失败告警）与 `test_daily_update_success_resets_failure_streak`;⑤ **消息模块有专测**——只追加且已读状态分离、**错误级推送不占信号额度**（`test_push_rules_keep_errors_outside_signal_quota`）;⑥ 新增页面（首页/数据/消息中心/骨架）**零内部代号**（grep 实证）;⑦ **既有菜单零回归**——17 个页面文件保留,本轮 diff 仅触及 DataPage/HomePage,旧页未动;⑧ 全量 `517 passed`、`MODULAR_BASELINE_PASS`。**遗留（非阻断,下轮处理）**：既有页面的内部代号尚未清理（按规格属各自重做时处理）;首页内容为骨架级,待「首页」菜单专项讨论后填充。
...
### 任务 SIG-2：信号台页面与人工留痕（2026-08-14 立项,交付 Codex,**当前唯一优先任务**）

**背景（用户反馈,记录）**：「这个版本连个信号池都没有,干嘛要我部署」——**分批切错了**:SIG-1 只有 Pushover（每天 3 条）与后台账本,用户看不到其余十几个信号,无法回顾比较。**SIG-2 完成后与 SIG-1 一起部署,不要求用户为不可见的服务做部署。**

- **① 信号台页面（一级入口「信号」,视觉准绳 `PAGE2_MOCKUP.html` 的信号台与图库）**：
  - **今日信号列表**:每条一行——币种、方向、触发形态与放量倍数、趋势强度排名、参考进场/建议止损/1R·2R·3R、信号时刻、有效期倒计时;**已推送的标记出来**（区分「推过手机」与「仅列表可见」）。
  - **每条带图**:价格线 + 均线 + 同期大盘 + 成交量（信号根高亮）+ 进场与止损虚线,复用样稿图形语言;点开看更长窗口。
  - **历史信号**:按日期回看,含模拟单结果（赚亏、最大浮盈/浮亏、多久到的）——这是形态的素颜成绩单,也是你练眼的教材。
  - **限额可见性**:当日被 30 条限额截断的信号**单独列出并标明**,不藏起来。
- **② 人工留痕（最小闭环）**：每条信号两个按钮「**我做了**」「**放弃**」;放弃可选原因标签（没盯盘/形态不喜欢/环境不好/其他）。**不要求手工填成交价**——账户同步后系统按时间+币种+方向自动配对真实成交;配不上的成交标记为**计划外交易**。
- **③ 四条硬约束（机制化,非提醒）**：止损随单而生且只许朝盈利方向移;价格低于进场价时加仓入口不存在;**同标的同方向止损后冷却期**（冷却中信号照常显示但「我做了」按钮禁用并说明原因与解禁时间）;当日连续止损/亏损限额熔断。放宽任何限制需 24 小时冷静期生效。
- **④ Pushover 配置界面（用户 2026-08-14 指出「没有地方配置 pushover,等于完全用不了」——SIG-1 验收漏项,Claude 认领）**：账户/设置页新增推送配置区——填写 Pushover **User Key 与 API Token**（密码输入,保存后不回显,走既有凭证库加密,禁止明文入库入日志）、**「发送测试通知」按钮**（立即验证配置是否可用并显示成功/失败原因）、推送开关与当日已推条数、门槛与限额的当前取值（只读展示）。**没有配置入口的功能视为未完成。**
- **⑤ 账户解耦与防误绑（2026-08-14 用户定稿:方案甲 + 信号服务不依赖账户）**：
  - **信号服务核心零账户依赖**——扫描、模拟单、推送、页面、我做了/放弃标记,**未绑定任何账户即可完整运行**;这是默认状态,也保证不可能污染 TB4。
  - **账户绑定为可选增强**,唯一用途是**取回真实成交与模拟单对照做复盘**（自动配对、计划外交易识别、三账对比）。绑定后功能增量见规格表。
  - **必须绑定专用子账户（方案甲）**:与 TB4 实盘账户**物理分离**,避免双方成交与持仓互相污染（TB4 调仓会被误判为计划外交易、人工成交会触发 TB4 持仓偏差告警、两边风险水位与 30% 停机线计算同时失真）。
  - **防误绑护栏（机制化,非提醒）**:**系统必须拒绝把信号服务绑定到已存在 TB4 自动执行绑定的账户**,复用 ARCH-1 的「一账户至多一条 ACTIVE 绑定」数据库约束;冲突时给出人话原因与「请使用独立子账户」的指引。有专测。
- **⑥ 开机与可观测（补齐用户指出的「用不了」类缺口,全部为必需项）**：信号服务启用/停用开关;**服务健康区**（是否在跑、上次扫描时间、行情新鲜度、当日已推条数、错误计数）;**当前生效参数只读展示**（币池门槛、形态设置、每日限额、推送门槛）——用户须能确认系统正按什么规则跑;**推送深链**（Pushover 消息带链接,点开直达该信号详情）;**信号过期后的显示与可否补标记**须明确定义;**故障也走推送告警**（行情中断、扫描失败、服务停止、连续推送失败,与信号推送用不同优先级,不占用每日 3 条额度）;信号账本纳入备份清单。
- **⑦ 不在本批**：三账对比复盘与季度减法工作台（SIG-3）;先保证「开得了、看得到、点得了、配得上、留得下痕、坏了知道」。
- **验收结论（Claude，2026-08-14）：通过（`6a31a48` + `1c159c1` + `4d8fa3b`）。** 按「六项可用性闭环」逐条核实:
  1. **开得了**——服务启用/停用开关、Pushover 配置（密码输入、保存不回显、走凭证库加密）与**发送测试通知**按钮、可选账户绑定下拉,均在页面上;`test_pushover_configuration_is_encrypted_and_testable` 有专测。
  2. **看得到**——信号台/信号复盘双 Tab、逐信号卡含方向与价位、K 线图组件、**被限额截断的信号仍可见并标注**（`test_truncated_signal_cannot_be_taken_but_remains_visible`）、我做了/没做的分区回顾。
  3. **点得了**——我做了/放弃（含原因下拉）落只追加账本;冷却与熔断时按钮禁用且**给出人话原因与解禁时间**;过期信号不可补标记（专测）。
  4. **配得上**——绑定账户后自动配对真实成交、未配上者标记计划外交易（专测）;**防误绑护栏实证**:绑定已被 TB4 占用的账户抛错「该账户已绑定 TB4 自动执行。信号服务必须使用物理隔离的独立子账户」,且**事后 TB4 抢占该账户时同步会 fail-closed**（`test_bound_account_sync_fails_closed_if_tb4_claims_account_later`）——比规格要求更严。
  5. **留得下痕**——双账本（信号源 + 人工交互）均只追加,**两者被篡改都 fail-closed**（两项专测）;纪律放宽须等 24 小时（专测）。
  6. **坏了知道**——健康状态区（运行中/已启用但扫描超 30 分钟/已停用）、当前生效参数只读展示、**未部署时诚实空态**（`test_missing_signal_service_is_an_honest_empty_state`,明写「不会填充演示数据」）。
  另:零账户依赖成立（未绑定账户可完整使用）、深链高亮已实现、窄屏样式齐备。全量 `510 passed`、`MODULAR_BASELINE_PASS`。附带修复 `4d8fa3b`（生产无 DATA-1R 数据集时研究页优雅降级）。**SIG-1 + SIG-2 合并可部署。**
- **验收**：① 列表与限额截断项齐全且区分已推送;② 图形语言与样稿一致;③ 冷却期禁用逻辑有测试;④ 人工成交自动配对与计划外交易标记有测试;⑤ 记录只追加;⑥ 人话与状态语言合规;⑦ 全量测试绿 + 基线 PASS + TB4 零触碰;⑧ Windows `npm check/build` + 窄屏实测。

### 生产现状与升级（2026-08-14 用户告知,待办）

**现状**：服务器**已部署并在运行**;实盘为**自动执行,但仍是 1 倍杠杆的旧版本（LIVE-SMALL V2 口径）**,尚未升级到 V3（目标仓位 ×3、12 市场逐仓 3x）。仓库代码已远超生产版本。

**待用户确认的两项事实**（决定升级方案）：① TB4 paper 前向是否已初始化并计时、已跑多久;② 生产服务器当前 commit。

**升级纪律（Claude 拟定,执行前需用户在场）**：
1. **两件事必须分开做,不得顺手合并**——(甲) 代码升级到当前版本;(乙) LIVE-SMALL V2→V3 的风险提升。
2. **甲的红线:paper 前向账本与起点绝对不动**。升级只换代码,`var/forward/tb4/` 原样保留;重启后必须 `verify_modular_baseline.py` 通过且账本验链通过、前向进度连续,否则回滚。**重新初始化＝前向计时作废,永久损失。**
3. **乙是实质风险提升**（目标仓位放大 3 倍、逐仓 3x）,必须走完整启用流程:账户准备 → 12 项预检 → 确认语;且须在用户明确决定后单独执行,不得作为升级副作用。协议 `LIVE_SMALL.md` V3 已冻结,V2 授权在升级时自动失效（`live_pilot_control` 有 fail-closed 降级逻辑,升级后会要求重新授权——属预期行为,非故障）。
4. 升级前备份 `var/forward/` 全部账本与 MySQL dump。
...
- **验收结论（Claude，2026-08-14）：通过（`c3ffe9b`）。** 逐条核实:① **信号定义与冻结范围一致有实证**——`sig1_scope_replay` 回放显示两族事件数与 RC-0B 参考值逐一相等（突破 29,311 / 超跌 9,183,`reference_count_match: true`）,推送门槛绑定 RC-0B 的 p90 趋势强度且 `threshold_match: true`;② **限额外信号照样入账**有专测（`test_all_signals_create_trades_even_beyond_daily_30_scope`）;③ 模拟单执行口径与冻结定义一致（次根开盘进、跳空取劣价、止损优先于时间退出、32 根兜底,各有专测）;④ 账本重启恢复与篡改检测有测试;⑤ **推送凭证只在投递时解析且不回传、网络错误信息被脱敏、冻结契约内无明文凭证**（三项专测）;⑥ 推送上限 3 条/日与跨日重置有测试;⑦ **推送失败不阻塞落盘并重试**（故障注入）;⑧ 全量 `495 passed`、`MODULAR_BASELINE_PASS`。**SIG-1 可部署。**

### 任务 SIG-6：策略页与研究页重做——从「陈列研究成果」改为「用户能动手 + 用图说话」（2026-08-15 用户反馈,交付 Codex,与 SIG-3/4/5 同批）

**用户原话**：「前端策略和研究页面根本就和用户没什么关系,也看不懂,参数都是你定的,也没有任何图形。」**诊断成立**:两页均按「陈列 Claude 的研究成果」设计——研究页是候选档案馆（M0/F1/G1/G2、数据集指纹、冻结定义）,策略页是参数表（12 市场、动量 14/28/56/84/168、vol28…）。**用户打开后无事可做,且参数脱离图形无法理解**;交易理解发生在图上,而两页一张图都没有。

- **研究与策略必须拆为两个独立入口（2026-08-15 用户纠正,此前合并为一页是错误）**：二者是**两种状态、两种用法**——
  | | 研究 | 策略 |
  |---|---|---|
  | 定义 | 尚未成型的探索方向（如「量价关系」） | 已成型、可运行的规则 |
  | 内容 | 想法、试验、图、观察 | 指标、参数、过滤条件、进出规则 |
  | 用户在此做什么 | **试**——改条件、跑回放、找规律 | **查与配**——看它在找什么、调参数、启停 |
  | 状态 | 未定型,随时推翻 | 定型,改动留痕 |
  导航为两个平级入口,不再是同一页的 Tab。
- **「毕业」由用户决定,不需任何审批**：在研究页试出满意的一套条件后,点「保存为策略」即进入策略列表——获得名称、指标、参数、过滤条件与进出规则,可被启用。**这与「参数由用户自己配」是同一条原则的延伸;Claude 与 Codex 不担任准入审批者。**
- **研究页 = 探索台**：主体即 SIG-4 + SIG-5 的试验能力——**配置条件 → 回放过去 7/30 天 → 信号图墙 → 用户自行判断 → 满意即保存为策略**;当前唯一主题「量价关系」。**首屏必须是可操作的探索界面。**
- **内部代号零出现（2026-08-15 用户指出「M0、F1 这些名词没有任何意义,用户根本不知道干嘛」——此前「降级为折叠区」的处置仍然错误,折叠不等于消失）**：
  - **历史研究档案区整个移除**,不是折叠。死掉的候选对用户零操作价值;其意义仅限审计溯源,而溯源的归宿是仓库文档（`PROJECT_PROGRESS.md` / `docs/evidence/`）,不是用户每天打开的页面。若确需一句交代,只保留人话摘要（如「此前检验过若干想法,均未通过」）,不列代号、不可展开。
  - **术语黑名单扩展至全部内部代号**：`M0 / F1 / G1 / G2 / V1-V4 / TB1-TB4 / TB-R / R-0 / RB-* / RC-* / SIG-* / MOD-* / PAGE-* / DATA-*` 等一律**不得出现在任何用户界面文案**（含标签、提示、空态、错误信息、图例）;策略对用户只有人话名称（如「趋势策略」「突破信号」）。验收以 grep 全模板断言,发现即打回。
- **策略中心 = 全部策略的统一列表（2026-08-15 用户明确规定结构）**：**自动策略与信号策略同列一张表**（现状:趋势策略在策略页、突破/超跌/持续强势散在信号页,无统一入口——本质都是「一套找机会的规则」,须统一描述）。每个策略一张卡,点开为详情,**固定含以下六节,全部人话,禁用内部代号与研究流程术语（「预注册」「锁箱」「冻结」「verdict」等一律不得出现）**：
  1. **这个策略在找什么**——一句话描述 + 图例（成功/失败各若干真实样本）;
  2. **用到哪些指标**——列出计算项（如趋势强度、ATR、成交量比、通道高低点、长周期均线）,每项配一句人话解释;
  3. **参数取什么值**——逐项当前值,可调项直接链到配置面板（SIG-5）,不可调项注明原因（如「与实盘运行绑定,变更需重新授权」）;
  4. **前置过滤（找信号之前排除什么）**——币池可交易性条件、长周期趋势状态要求、黑白名单等;
  5. **后置过滤（找到信号之后筛掉什么）**——崩塌熔断、每日限额、排序取前 N、冷却期等;
  6. **进场与出场规则**——何时进、止损放哪、何时出。
  另附运行状态（是否启用、今日已产生多少信号/持仓）与「查看回放」入口。
- **策略详情的图形部分（参数降级为技术详情）**：
  1. **权益曲线**——该策略自启动至今的净值与回撤;
  2. **当前持仓画在价格图上**——每个持仓币种一张图,标出进场位置、当前位置、为什么持有（方向与权重）;
  3. **历史调仓点标注在价格图上**——持多/持空区间底色 + 调仓时刻点（沿用 `PAGE2_MOCKUP.html` 的图形语言）;
  4. **参数表折叠**为「技术详情」,不占主视野;五步运转说明保留（已验收合格）。
- **原则（适用于两页）**：**图在前、数字在后、参数最后**;凡是用户不能动手的内容一律不占首屏;所有可调项与 SIG-5 的配置面板联动。
- **验收**：① 研究页首屏为可操作探索界面（模板断言:含配置与回放入口,非只读档案）;② 策略页首屏含权益曲线与至少一张持仓价格图;③ 参数表位于折叠区;④ 图形语言与样稿一致;⑤ 人话与状态语言合规;⑥ 全量测试绿 + 基线 PASS + TB4 零触碰。

### 任务 SIG-5：用户自主配置与自定义筛选条件（2026-08-15 用户提出,交付 Codex,与 SIG-3/SIG-4 同批）

**用户原话**：「这些参数就应该在研究的时候用户自己来配,筛选条件用户自己加。」**方向正确,此前架构错在用「研究的纪律」管「用户的偏好」**——「证明策略赚钱」是结论,必须预注册冻结;「决定我每天看什么」是偏好,理应用户自主。信号服务属后者,过去每改一个数都需经 Claude 走冻结流程,既慢且未必更准（TUT/ACE 实证）。**配置权交给用户,纪律改由「全量留痕 + 事后诚实评估」承担。**

- **① 可调参数（全部开放给用户,页面直接改）**：币池可交易性门槛与窗口;三类信号各自的阈值（通道长度、放量倍数、跌幅、持有期）;趋势过滤开关与定义;崩塌熔断线与「近期高点」窗口;每日限额与推送条数;三类信号独立开关;币种黑白名单。**每项带人话说明与当前值。**
- **② 自定义筛选条件构建器（核心新能力）**：用户可自行添加条件,形式为「字段 + 比较符 + 值」,支持与/或组合与分组。**可用字段至少包含系统已计算的全部量**:方向对齐趋势强度、效率比、插针度、相对成交量、成交量趋势（多窗口）、4h/日线长周期状态、距近期高点回撤、ATR 相对值、上市天数、流动性分层、UTC 时段、币种。条件对三类信号可分别设定或全局生效。
- **③ 与回放预览联动（依赖 SIG-4）**：任何参数或条件改动**立即触发过去 N 天回放**,并排显示改动前后差异（信号增减、币种构成、逐条差异）。**先看后定,不必上线试错。**
- **④ 纪律：不限制配置,而是如实记录与事后裁决**：
  - 每次配置变更＝**带时间戳的新版本**,写入只追加配置账本（谁、何时、改了什么、变更说明）;
  - **模拟账全量记录**,且额外维护一份**「完全不过滤基线账」**（记录未经用户条件过滤的全部候选信号及其模拟结果）——**这是关键**:几个月后可直接算出「用户的自定义条件到底避开了垃圾,还是连好机会一起筛掉了」;
  - 回放与配置页固定标注「历史回放不预测未来;条件的真实价值由未来数据上的模拟账裁决」;
  - **不提供收益排行榜、不提供参数推荐、不按历史收益排序条件**（沿用 SIG-4 红线,防止把配置工具变成拟合工具）。
- **验收**：① 全部列出参数可在页面调整并即时预览;② 条件构建器支持字段/比较符/与或组合,非法条件有清晰报错;③ **无过滤基线账与用户过滤账并行记录**（专测）;④ 配置变更版本化留痕（专测）;⑤ 无排行榜/推荐（模板断言）;⑥ 全量测试绿 + 基线 PASS。

### 任务 SIG-4：信号规则可见化与回放预览（2026-08-15 用户反馈「信号策略是个黑盒」,交付 Codex,与 SIG-3 同批）

**用户原话**：「我们现在的信号策略就是一个黑盒,根本不知道推送的是什么信号、长什么样,只有推出来了才能看到。」**问题成立且是设计缺失**——用户无法在信号推送前理解规则行为,只能事后发现是垃圾再反馈,形成「上线→骂→改」的救火循环。

- **① 每类信号的图解说明（替代参数说明）**：突破 / 超跌 / 持续强势各一节,用**真实历史样本的图**说明「这类信号长什么样」,**必须同时给成功与失败的例子**（各 3~5 张,取自冻结数据集,标注信号触发点、进场、止损、后续走势）;参数表降级为折叠的技术细节。**页面主体是图不是数字。**
- **② 规则回放（核心功能）**：选时间段（默认过去 7 天,可选 30 天）→ **按当前生效规则完整跑一遍** → 列出该期间会推送的全部信号:逐条带图、带事后结果（最大浮盈/浮亏、若机械执行的收益）、带被限额截断的部分。**目的:部署前即可看清「这套规则会给我什么」。**
- **③ 改动即时预览**：调整任一参数（币池门槛、形态阈值、崩塌熔断线、趋势过滤开关、三类信号开关）后,**立即重算过去 N 天的回放结果**并与当前规则并排对比:信号数量变化、币种构成变化、逐条差异（新增哪些、消失哪些）。**改规则从「上线后祈祷」变为「先看后定」。**
- **④ 当前生效规则的人话展示**：不是 JSON——用一段话 + 图说清「现在系统在找什么」,含各阈值当前值与最近一次变更时间。
- **纪律红线（必须写进页面）**：**回放用于理解规则行为,不得用于挑选收益最优参数**——后者即在历史上拟合,是本项目全套预注册纪律所防范的。故页面**不提供参数收益排行榜、不提供「最优参数」推荐、不按收益排序参数组合**;回放结果页固定标注「历史回放不预测未来,规则变更的真实效果由信号模拟账在未来数据上裁决」。
- **验收**：① 三类信号各有成败图例;② 回放结果与 SIG-1 引擎同源（同一段历史,回放输出与实时引擎逐条一致,专测）;③ 改动预览的并排差异正确（专测）;④ 无收益排行榜/最优推荐（模板断言）;⑤ 人话与状态语言合规;⑥ 全量测试绿 + 基线 PASS。

> **2026-08-17 起，SIG-3 的唯一有效规格是自足任务卡 `docs/tasks/SIG3_SIGNAL_SCOPE.md`**（含全部六项改动、已填定的三个待定数值、顺序红线与验收清单）。下方历史条目仅保留为讨论过程记录,实现请勿据此施工。

### 任务 SIG-3：推送范围修正（2026-08-14 生产反馈触发,交付 Codex,**当前唯一优先**）

**用户生产反馈（原话）**：「一个好信号没有,推的都是垃圾信号,那种超跌反弹的,在趋势上都是一路向下的还要推;真正不断攀高、成交量不断上升的币一次都不推。」**两条批评均成立,根因在 Claude 的范围设计,记录如下:**
- **根因一（漏用已有数据）**：RB-2 已测出超跌族在**长周期 DOWN 状态四组参数全负**;Claude 当时的「长周期趋势门不成立」结论是针对**突破族**（三状态皆负）,未把超跌族的 DOWN 组差异落进推送范围,导致逆势抄底信号照推。
- **根因二（收紧方式选错,造成系统性偏差）**：为把突破族日均从 59.8 压到 16.1,Claude 选了「放量 ≥4.0 倍」,依据是 RC-0B「收紧形态不带信息、只线性砍量」。**该依据只看总量保留率,未看被砍掉的是哪一类**:4 倍放量选中的是新闻脉冲式暴量,而「稳步攀高 + 成交量逐步放大」的健康趋势单根 15m 几乎不可能达到 4 倍,**被系统性排除**。

- **修正一 · 超跌族加长周期过滤（有数据支持,2026-08-15 用户再次强调后加强）**：**超跌信号仅在长周期状态为 `UP` 时推送**;`DOWN` 与 `RANGE` 均不推。依据:RB-2 实测 DOWN 组四参数全负、UP 组 `−0.014~+0.032R`（约打平）、RANGE 组 `−0.049~+0.153R`（不稳定）——**除「上升趋势中的回调」外均不值得占用用户注意力**。长周期定义沿用 RB-2（4h 收盘在 MA50 上方且 20/60 日收益同向为 UP,三项反向为 DOWN,其余 RANGE）。
- **修正零 · 币池维度本身选错（2026-08-15 生产实证,最根本的一条）**：用**成交额**做准入,实际筛的是「当下最热闹」,而热闹既可能是启动也可能是崩塌——**成交额不含方向信息**。实证对比（合约成交额 3 日中位 vs 实际走势）:**TUTUSDT 3.44 亿→过 2 亿门槛→入池→两天内推 26 个超跌信号（22 亏 / 4 赚,均值 −5.16%）,而该币 5 日累计下跌 85%（−46.6/−28.9/−12.4/−47.3/−14.4%）,每天暴跌都伴随巨额恐慌成交额**;**ACEUSDT 270 万→被门槛挡在外→零信号,而其两日上涨 192%**。结论:**门槛数字不是问题,拿成交额当准入维度才是问题。**
  - **修正 0a · 准入只回答「能不能交易」**：改用**长窗口低门槛**（建议 30 日中位成交额 ≥ 200 万,数值待用户定）,仅排除无深度的僵尸币与新上市无历史者;准入不再承担「值不值得看」的职责,故不再随行情剧烈进出。
  - **修正 0b · 「值不值得看」全部交给信号定义,且信号必须带方向**：持续强势（在涨）、突破（向上突破）、超跌（仅长周期向上时的回调）。成交量放大作为**信号内容**用滚动窗口实时计算,不再作为准入条件、不再等完整 UTC 日。
  - **修正 0c · 超跌信号新增「崩塌熔断」（比长周期状态更直接）**：**距近期高点回撤超过阈值（建议 30%）即不推超跌**,无论长周期状态如何。实证依据:TUT 自 0.088 跌至 0.030（−66%）仍在持续推送;且其长周期状态因先前暴涨而被判为 RANGE（20/60 日收益仍为 +97%/+190%）,靠长周期过滤拦住它属侥幸——**「离高点跌了多少」才是对崩塌最直接的度量**。
- **修正一 b · 顺序红线（重要）**：**趋势过滤必须先于币池放开上线**。放宽币池门槛会引入大量长期阴跌的小市值币,它们高频满足「4 小时跌 10% + 一根止跌 K 线」,若先放开币池再加过滤,垃圾超跌信号将暴增。**部署顺序:先修正一,再改币池。**
- **修正一 c · 三类信号独立开关**：突破 / 超跌 / 持续强势各自可单独启用停用（页面开关,写入配置并留痕）。符合系统定位——范围由系统缩,取舍由用户定;用户可随时整类关闭超跌。
- **修正二 · 突破族放量门槛 4.0 → 2.0**：放弃「用极端放量控量」;数量由既有每日限额 30 承担。
- **修正三 · 新增第三类信号「持续强势」（用户实际想要的类型,亦即 RC-1 概念的落地）**：状态型信号,非事件触发。条件（全部只用已收盘数据,具体阈值由 Codex 在契约中冻结并记录依据）：① 长周期状态为 `UP`;② **方向对齐趋势强度**处于高分位——**这是 RC-0 中唯一被证实富集 >1（1.73~1.84×）的指标**;③ **成交量持续放大**（近 3 日均量 / 近 10 日均量 高于阈值,取「持续上升」而非「单根暴量」）;④ 价格处于近 N 日高点附近。同一标的须有冷却,避免趋势期内每根 K 线重复推送。
- **纪律与诚实边界（必须写进产品文案与报告）**：**本次是「推送范围」调整,不是被证明更赚钱的规则**;修正一有 RB-2 数据支持,修正二三是纠正设计偏差与补齐用户所需类型,**均未经独立验证**。**所有信号（含修正前后）继续全量进入只追加模拟账**,由日历时间累积的样本外记录最终裁决三类信号各自的价值;范围变更须在账本中留下版本标记,便于日后分段评估。
- **验收**：① 超跌 DOWN 状态零推送有专测;② 放量门槛与新类型阈值写入冻结契约且报告实际日均量;③ 「持续强势」信号有同标的冷却、不重复刷屏（专测）;④ 三类信号在页面与推送中可区分标注;⑤ 模拟账记录含**范围版本标记**（专测）;⑥ 全量测试绿 + 基线 PASS + TB4 零触碰。

### 信号服务推送范围（**用户 2026-08-14 定稿,冻结** —— 见上方 SIG-3 修正）
...
### SIG-2 完成记录：信号台与人工留痕（2026-08-14，Codex）

**目标**：在不改变 SIG-1 信号定义和模拟账的前提下，落地可见、可点、可追溯的信号工作台。本节是实现记录，以上方最新 SIG-2 正式任务为唯一验收契约。

**冻结范围**：

- 新增“信号”一级页面，包含信号台和信号复盘；信号卡以紧凑 K 线为主体，在同图展示成交量、进场、止损和小型方向箭头，逐根可查看涨跌幅与振幅。
- 人工只对正式候选登记“我做了 / 我跳过”；可选绑定独立子账户后，由账户同步自动配对 Binance 真实成交，但始终不自动下单。
- 强制纪律：选择时必须带有效止损、禁止亏损加仓、同交易对同方向止损后冷静 4 小时、计划外成交单独识别、连续 3 笔亏损或当日累计 `-3R` 熔断；纪律放宽至少延迟 24 小时。
- 三本账试算继续保留用于界面参考；正式三账比较按最新契约归入 SIG-3，不作为 SIG-2 完成条件。
- SIG-1 与 SIG-2 分账存储并各自保持只追加哈希链；源账缺失或校验失败时诚实降级，不制造示例信号。

**实现记录（Codex，2026-08-14）：SIG-2 已按最新正式契约完成。** 后端提供信号台、人工决定、Pushover 安全配置/测试、服务启停、可选成交对照账户及纪律变更接口；独立 `ORBIT_SIG2_INTERACTION_LEDGER_V1` 保存只追加人工与运行控制事件。Pushover Token/User Key 经凭证库加密且不回显；推送包含可直达并高亮具体信号的深链，扫描、行情和连续推送失败使用独立故障告警。信号核心保持零账户依赖；绑定账户在保存及每次同步时都检查 TB4 活跃占用，账户同步自动配对真实进出场，不能关联正式信号的成交和持仓标记为计划外。后端与页面共同执行 30 分钟过期、止损随单、禁止亏损加仓、同向止损后 4 小时冷静期、连续亏损/当日 `-3R` 熔断及 24 小时放宽延迟。页面展示方向、家族、相对成交量、趋势强度同族排名、进场/止损/1R/2R/3R、时间与倒计时、推送状态；紧凑图同屏展示 K 线、均线、成交量和同期 BTC，并支持放大。运行观测展示开关、扫描时间、行情新鲜度、推送计数、错误和冻结参数；备份清单覆盖 SIG-1/SIG-2 双账本。三账界面保留为试算，正式验收归 SIG-3。设计见 `docs/design/SIG2_SIGNAL_DESK.md`。验收：SIG-2/SIG-1/K 线专项 `27 passed`，后端全量 `511 passed`（仅 2 条既有 Pydantic 弃用警告），前端 check/build 通过，`MODULAR_BASELINE_PASS`，TB4 受保护文件零改动；桌面与 390px 窄屏完成浏览器走查。

**网页预警与配置入口修正（Codex，2026-08-14）：** Pushover 凭证统一保留在“账户”页面，信号台删除重复配置，仅展示推送状态；信号页新增“网页预警”页签，从真实事件账本展示扫描失败/超时、推送失败、计划外交易、TB4 账户冲突、纪律熔断与冷静期，并根据后续心跳或测试成功把历史故障标为“已恢复”。验收：信号专项 `25 passed`、后端全量 `512 passed`（仅 2 条既有 Pydantic 弃用警告）、前端 check/build 通过、`MODULAR_BASELINE_PASS`，桌面浏览器完成真实渲染走查。
