...
### 任务 DATA-1：含退市合约的全市场历史数据集 + 时点币池（优先级：高，交付 Codex；形态/热门/宽币池研究的硬前置）
...
- **改动**：
  1. **合约全集枚举**：以 Binance 公开历史归档（`data.binance.vision`,含已退市合约的 K 线/Funding 文件）为主数据源,枚举 USDT-M 永续**历史上出现过的全部**合约;逐合约登记元数据:上市日（首根数据）、退市日（末根数据 + 状态判定）、当前状态。归档为公开 CDN,先在本机探测可达性;若同样被 451 拦截,工具须可在 Windows 侧/部署主机独立运行。
