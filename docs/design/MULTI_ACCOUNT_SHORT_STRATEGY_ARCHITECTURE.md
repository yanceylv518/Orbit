...
状态：`PARTIALLY_IMPLEMENTED`（ARCH-1 控制面已实现；数据、回测、worker 与执行阶段仍为目标架构）
最后更新：2026-08-10

本文定义 Orbit 从单一 TB4/旧策略运行闭环升级为多账户短线策略研究、回测、Paper 与 Live 平台的正式目标架构。本文不是完整能力声明；当前仅 ARCH-1 的附加数据模型、TB4 只读投影、迁移工具和管理员查询接口已落地，且尚未在生产 MySQL 执行迁移。其余目标组件、目标表或目标状态机在迁移完成并通过验收前均不得对外宣称已可用。
...
- 每个新短线策略账户最多同时持有 3 个非零币种仓位；这是硬上限，不是目标持仓数。现有 TB4 是冻结的 12 市场 legacy 策略，迁移期不得反向套用三仓政策或修改其运行语义；新建的非 legacy 实例必须绑定三仓政策。
...
- 第一阶段以 15m 为最细原始 K 线和信号周期，1h/4h 由完整的 15m 子根按 UTC 边界本地聚合；5m/1m 不在当前范围。若 R-0 后证明需要更细粒度，必须重新评估数据量、成交模型和运行架构并单独立项。
...
- 新短线策略账户最多 3 个非零币种仓位；人工仓位和未知归属仓位也计入。TB4 legacy 使用与冻结规格一致的只读兼容风险政策，直至独立迁移或退役决策。
...
第一版最少数据：15m OHLCV、成交笔数/主动成交量（若上游提供）、资金费率、标记价格、交易规则和合约状态。1h/4h 聚合根必须由四根/十六根连续 15m 子根生成，缺根时标记 `INCOMPLETE`，不得静默拼接。历史最佳买卖价与深度仅在数据源覆盖和质量完成独立可行性验证后才能进入成本模型；在此之前使用预注册、按流动性分层的保守点差与冲击缓冲，并明确其证据等级。仅有 K 线收盘价不足以声称已经验证真实可成交性。
...
账户级 `PortfolioRiskPolicy` 在任何开仓意图落 outbox 前同步判定。新短线策略政策强制三仓；TB4 legacy 只登记当前冻结风险语义，不通过本次迁移改变执行行为：
...
PortfolioRiskPolicy
- id, version
- max_open_symbols（新短线策略固定为 3；legacy 按冻结定义登记）
...
- 新短线策略持仓名额的 `RESERVED/OPEN/CLOSING/UNKNOWN` 数量不得超过 3；风险退出不因名额上限被阻断。legacy 实例按其只读兼容政策校验，禁止借兼容政策创建新的非 legacy 多仓实例。
...
ARCH-1 采用 expand-and-verify：先通过 `backend/scripts/setup_mysql.py` 幂等创建六张附加控制面表，再运行 `backend/scripts/migrate_strategy_control_plane.py`。迁移命令默认仅校验前置条件；显式传入 `--apply` 才在事务中读取 `app_runtime_state` 的持久化 LIVE-SMALL 控制记录并写入 TB4 只读投影。它不写旧执行表，也不会把短线三仓政策套到 TB4。

MySQL DDL 会隐式提交，因此“回滚”不是伪装成同事务撤销建表：切换读取前可回滚应用版本并保留未启用的附加表；种子写入失败则事务回滚。删除附加表属于独立、经审批且在备份验证后的运维动作。迁移后必须核对 TB4 两个冻结哈希、账户绑定、风险政策类型以及现有 LIVE-SMALL 执行路径 zero-diff，校验失败时禁止切换读取。

Windows 检查与执行命令：

```powershell
.\.venv\Scripts\python.exe backend/scripts/migrate_strategy_control_plane.py
.\.venv\Scripts\python.exe backend/scripts/migrate_strategy_control_plane.py --apply
```
