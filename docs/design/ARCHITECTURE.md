...
> 范围说明：本文记录现有工程分层和第一阶段边界。面向“多账户并行、每账户单策略、每日动态流动性币池、每账户最多三仓”的短线策略平台升级，目标架构见 `MULTI_ACCOUNT_SHORT_STRATEGY_ARCHITECTURE.md`。该方案当前状态为 `PROPOSED`，不得视为已实现能力。

> 后续模块决策：数据、研究、策略、运行、执行、风险、复盘和账户的业务边界、统一数据契约及渐进迁移顺序，以 `SYSTEM_MODULE_REDESIGN.md` 为准；本文继续定义技术分层与依赖方向。
...
旧双网格策略仍按本节分散到工作台、执行计划、币种视图和风控中心。TB4 成为正式 paper/受限实盘策略后，需要独立“策略中心”承载冻结定义、结构化回测证据和运行摘要；其只读边界、数据模型、API 与迁移计划以 `docs/design/STRATEGY_CENTER.md` 为准。策略中心不恢复已删除的静态配置页，也不承担调参或下单。
