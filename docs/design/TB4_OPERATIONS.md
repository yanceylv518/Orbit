...
版本：2026-07-31（V4：LIVE-SMALL V3 使用 3 倍目标与逐仓 3x；TB4 判定纪律不变）
适用对象：平台管理员（自用）
...
- ✅ **LIVE-SMALL 小资金实盘自动执行已实现**（LIVE-1/2/3）：与 paper 前向**并行**运行，受 `LIVE_SMALL.md` V3 协议管辖（500 USDT、TB4 目标乘 3、逐仓 3x）。
...
2. **启动后什么都别动**：不调参数、不提前下结论、不因早期波动改变任何冻结项。前向测试最大的敌人是手贱。
   - **与小资金实盘的关系（澄清，避免误读）**：按 `LIVE_SMALL.md` V3，500 USDT 实盘目标是原始 TB4 权重的 3 倍，并与未放大的 paper 前向并行。两者风险倍数不同，必须分账复盘；TB4 paper 的 12 个月判定纪律不受影响。
...
4. **准备实盘账户**：输入页面要求的确认短语后，系统先确认 Binance 账户无挂单、无持仓，
   再将交易所切换为单向持仓、把 12 个策略市场设置为逐仓 3x 并确认自动追加保证金关闭，
   最后将 Orbit 账户明确设置为主网、
   `dry_run=false`。该动作不下单；交易所任一步失败均不会开放实盘。
5. **运行生产预检**：系统自动同步账户，并逐项验证账户状态、真实单向持仓、至少 500 USDT
   钱包权益、规则新鲜度、无挂单、无既有持仓、12 市场逐仓 3x、自动追加保证金关闭和 Binance `/order/test`
   合约交易权限。冻结清单尚未生成时显示“等待”，但不阻止管理员提前授权布防。
6. **启用自动执行**：填写新的不可复用执行批次（epoch），再输入
   `ENABLE LIVE SMALL V3`。系统会重新预检并持久化授权状态。若清单尚未生成则进入 `ARMED`
   （已布防），本操作本身不会下单；只有首份冻结清单生成且再次通过逐轮执行闸门后，系统
   才可能发送真实市价单，不需要重启服务。
...
- **PASS**：授权**讨论 LIVE-SMALL 规模升级到"正式规模"**（小资金实盘按 `LIVE_SMALL.md` V3 已在并行运行）；升级仍是需要显式确认的独立决定，且须以新协议版本预注册。
