# Orbit 文档目录

## product

- `Dynamic Dual Grid V1 开发需求.pdf`：原始产品需求。
- `Dynamic Dual Grid V1 修正版产品技术方案.md`：当前产品与技术方案说明。

## design

- `ARCHITECTURE.md`：工程架构边界说明。
- `MULTI_ACCOUNT_SHORT_STRATEGY_ARCHITECTURE.md`：多账户、单账户单策略、动态流动性币池、三仓上限的短线策略平台目标架构（`PROPOSED`，尚未实现）。
- `STRATEGY_LOGIC.md`：策略逻辑数学化重构设计，包含净敞口 Δ、趋势生命周期、风控 guard 和仿真验收规范。
- `dynamic-dual-grid-product-design-cn.png`：中文控制台设计参考图。
- `dynamic-dual-grid-product-design.png`：英文控制台设计参考图。
