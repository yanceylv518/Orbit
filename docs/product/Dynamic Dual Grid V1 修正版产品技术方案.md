...
工程状态（2026-07-09）：已新增 `domain/risk/guards.py`，`plan_only` 计划生成和 dry-run 引擎已共用同一套动作前置风控。当前已覆盖 gross `ONLY_REDUCE`、计划模式/加仓开关、symbol 级 `STOPPED`；组合级回撤、C7 自融资账本和行情/持仓快照新鲜度仍待接入。
