...
class LiveReconciliationServiceTest(unittest.TestCase):
    ...
    def test_equity_observations_are_append_only_and_normalized_to_first_sync(self):
        ...
        self.assertEqual(result["points"][-1]["exposure_multiplier"], 3.0)
        self.assertEqual(result["points"][-1]["initial_leverage"], 3)
        self.assertEqual(result["points"][-1]["margin_type"], "ISOLATED")
